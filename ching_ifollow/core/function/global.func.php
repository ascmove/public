...
function web_url($segment, $params = array()) {
	...
	$url = $_W['siteroot'] . 'web/index.php?c=site&a=entry&m=ching_ifollow&';
	if(!empty($do)) {
		$url .= "do={$do}&";
	}
	if(!empty($ac)) {
...
function app_url($segment, $params = array()) {
	global $_W;
	list($do, $ac, $op) = explode('/', $segment);
	$url = $_W['siteroot'] . 'app/index.php?i='.$_W['uniacid'].'&c=entry&m=fightgroups&';
	if(!empty($do)) {
		$url .= "do={$do}&";
	}
	if(!empty($ac)) {
...
function ching_log($message, $data = ''){
	if ($data) {
...
function ching_template($filename,$flag='') {
	global $_W;
	$name = 'ching_ifollow';
	if (defined('IN_SYS')) {
		$template = $_W['chingsetting']['webview'];
		if (empty($template)) {
			$template = "default";
		} 
	...
	if (!is_file($compile) || filemtime($source) > filemtime($compile)) {
		ching_template_compile($source, $compile, true);
...
/********************app端自定义页面加载************************/
function ching_template_page($id, $flag = TEMPLATE_DISPLAY) {
	global $_W;
	$page = ching_page($id);
	if (empty($page)) {
	...
	$content = ching_template_parse($page['html']);
	file_put_contents($compile, $content);
	switch ($flag) {
		case TEMPLATE_DISPLAY:
		default:
			extract($GLOBALS, EXTR_SKIP);
			include ching_template('common/app_header');
			include $compile;
			include ching_template('common/app_footer');
			break;
		case TEMPLATE_FETCH:
			extract($GLOBALS, EXTR_SKIP);
			ob_clean();
...
/********************web端自定义页面加载************************/
function ching_tpl_wappage_editor($editorparams = '', $editormodules = array()) {
	global $_GPC;
	$content = '';
	load()->func('file');
	$filetree = file_tree(IA_ROOT . '/addons/fightgroups/web/view/default/wapeditor');
	if (!empty($filetree)) {
		foreach ($filetree as $file) {
			if (strexists($file, 'widget-')) {
				$fileinfo = pathinfo($file);
				$_GPC['iseditor'] = false;
				$display =  ching_template('wapeditor/'.$fileinfo['filename'], TEMPLATE_FETCH);
				$_GPC['iseditor'] = true;
				$editor =  ching_template('wapeditor/'.$fileinfo['filename'], TEMPLATE_FETCH);
...
/********************web端自定义页面加载************************/
function ching_tpl_ueditor($id, $value = '') {
	$s = '';
	if (!defined('TPL_INIT_UEDITOR')) {
		$s .= '<script type="text/javascript" src="../addons/fightgroups/web/resource/components/ueditor/ueditor.config.js"></script><script type="text/javascript" src="../addons/fightgroups/web/resource/components/ueditor/ueditor.all.min.js"></script><script type="text/javascript" src="../addons/fightgroups/web/resource/components/ueditor/lang/zh-cn/zh-cn.js"></script>';
	}
...
function ching_template_compile($from, $to, $inmodule = false) {
	$path = dirname($to);
	if (!is_dir($path)) {
		load()->func('file');
		mkdirs($path);
	}
	$content = ching_template_parse(file_get_contents($from), $inmodule);
...
function ching_template_parse($str, $inmodule = false) {
...
function ching_template_addquote($matchs) {
	$code = "<?php {$matchs[1]}?>";
...
function finance($openid = '', $paytype = 0, $money = 0, $trade_no = '', $desc = ''){
		...
		if (empty($openid)) {
			...
			$xml = array2xml($pars);
			$path_cert = IA_ROOT . '/attachment/fightgroups/cert/' . $_W['uniacid'] . '/apiclient_cert.pem';
			//证书路径
			$path_key = IA_ROOT . '/attachment/fightgroups/cert/' . $_W['uniacid'] . '/apiclient_key.pem';
			//证书路径
			if(!file_exists($path_cert)|| !file_exists($path_key)){
			$path_cert = IA_ROOT . '/addons/fightgroups/cert/' . $_W['uniacid'] . '/apiclient_cert.pem';
			$path_key = IA_ROOT . '/addons/fightgroups/cert/' . $_W['uniacid'] . '/apiclient_key.pem';
			}
			$extras = array();
...
function ching_message($msg, $redirect = '', $type = '') {
	global $_W;
	if($redirect == 'refresh') {
	...
	include ching_template('common/message', TEMPLATE_INCLUDEPATH);
	exit();
}

function ching_json($status = 1, $return = null) {
...
//		}
    ...
    function qrcode_logo($config, $path,$_wx_url, $is_merge=false){
        ...
        $matrixPointSize = ($config['logoqrwidth']>0 ? $config['logoqrwidth'] : 3);//生成图片大小
        if($matrixPointSize > 10) {
            $matrixPointSize = 20;
        }
        QRcode::png($_wx_url, $path, $errorCorrectionLevel, $matrixPointSize, 1);//生成二维码图片 无logo
        $logo = $config['qrcode_logo']=='../addons/ching_ifollow/template/images/110*110.png' ? realpath($config['qrcode_logo']) : realpath('../attachment/'.$config['qrcode_logo']);//上传至服务器的logo图片
        $QR = $path;//已经生成的原始二维码图
        if ($logo !== FALSE) {
        ...
        $base_save_path = IA_ROOT.'/attachment/'.CHING_NAME.'/'.$p;
        imagepng($QR, $base_save_path);//输出带logo的二维码图片
        $save_path = '../attachment/'.CHING_NAME.'/'.$p;
        ...
        if($is_merge) {
            ...
            $_save_path = IA_ROOT.'/attachment/'.CHING_NAME.'/'.$p;
