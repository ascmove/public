...
class Ching_ifollowModule extends WeModule {
	public function fieldsFormDisplay($rid = 0) {

	    global $_W;
        if(!empty($rid)) {
            $item = pdo_fetch("SELECT * FROM ".tablename('ching_ifollow')." WHERE rid = :rid", array(':rid' => $rid));
        }
		//要嵌入规则编辑页的自定义内容，这里 $rid 为对应的规则编号，新增时为 0
        $fansgroup = pdo_getall('ching_ifollow_fans_group', array('uniacid'=>$_W['uniacid']), array('id', 'name'));
        load()->func('tpl');
	...
	public function fieldsFormSubmit($rid) {
        ...
        $acid = intval($_W['acid']);
        load()->func('file');
        $path = IA_ROOT.'/attachment/'.CHING_NAME.'/'.date('Y').'/'.date('m').'/'.date('d').'/'.time().'.png';
        if(!is_dir(IA_ROOT.'/attachment/'.CHING_NAME.'/'.date('Y').'/'.date('m').'/'.date('d'))) {
            mkdirs(IA_ROOT.'/attachment/'.CHING_NAME.'/'.date('Y').'/'.date('m').'/'.date('d'));
        }
        $id = intval($rid);
        $uniacid = $_W['uniacid'];
        load()->func('communication');
        ...
        $config = cache_load('ching_ifollow_config');
        if(!$config) {
            message('请设置配置信息', 'referer', 'error');
        }
        $insert_data['model'] = $config['mode'];
        $info = pdo_fetch( 'SELECT * FROM '.tablename('ching_ifollow').' WHERE uniacid = :uniacid AND rid = :id' , array(':uniacid' => $uniacid,':id'=>$id));
        if( $info ){
            pdo_update('ching_ifollow', $insert_data, array('id'=>$id));
        } else {
            ...
            pdo_insert('ching_ifollow', $insert_data);
        }
	}
	...
	public function ruleDeleted($rid) {
        ...
        pdo_delete('ching_ifollow', array('uniacid'=>$_W['uniacid'], 'rid'=>$rid));
        pdo_delete('ching_ifollow_fans', array('uniacid'=>$_W['uniacid'], 'qrcode_id'=>$qrcode['id'], 'group_id'=>$qrcode['group_id']));
        pdo_delete('ching_ifollow_stat', array('uniacid'=>$_W['uniacid'], 'qrcode_id'=>$qrcode['id'], 'group_id'=>$qrcode['group_id']));
        pdo_delete('ching_ifollow_record', array('uniacid'=>$_W['uniacid'], 'qrcode_id'=>$qrcode['id'], 'group_id'=>$qrcode['group_id']));
	}
	...
	public function settingsDisplay($settings) {
		...
		if(checksubmit()) {
            ...
            if ($this->saveSettings($dat)) {
                cache_write('ching_ifollow_config', $dat);
