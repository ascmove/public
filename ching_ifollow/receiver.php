...
class Ching_ifollowModuleReceiver extends WeModuleReceiver {
	public function receive() {
        ...
        $table_user_info = pdo_get('ching_ifollow_fans', array('uniacid'=>$uniacid, 'openid'=>$open_id));
        if($message['ticket']) {
            $qrcode = pdo_get('ching_ifollow', array('uniacid' => $uniacid, 'ticket' => $message['ticket']));
        } else {
            $wx = json_decode($table_user_info['wx_data'], true);
            $qrcode = pdo_get('ching_ifollow', array('uniacid' => $uniacid, 'ticket' => $wx['wx_ticket']['ticket']));
        }
        if(!$qrcode) {
            return '';
        }
        if($table_user_info) {
            if($type == 'unsubscribe') {
                pdo_update('ching_ifollow_fans', array('cancel_sub_time'=>$message['time']), array('id'=>$table_user_info['id']));
            }
        ...
        if($type == 'subscribe') {
            if(pdo_tableexists('ching_ifollow_cash_record')) {
                $this->operation_cash($open_id, $qrcode['id']);
            }
    ...
    public function operation_cash($openid,$qr_id,$direct='')
    {
        global $_W;
        load() -> model('mc');
        ...
        if($config) {
            if($config['integral'] > 0) {
                $this->setCredit($openid, 'credit1', $direct.intval($config['integral']), array($uid, '场景二维码-扫码送积分'));
            }
            if($config['balance'] > 0) {
                $this->setCredit($openid, 'credit2', $direct.intval($config['balance']), array($uid, '场景二维码-扫码送余额'));
            }
        }
        $dit = $direct=='-' ? 0 : 1;
        $cash_log = array(
            ...
            'openid'=>$openid,
            'direct'=>$dit,
            'integral'=>$config['integral']?$config['integral']:0,
            'balance'=>$config['balance']?$config['balance']:0,
            'create_time'=>TIMESTAMP
        );
        pdo_insert('ching_ifollow_cash_record', $cash_log);
	}
    ...
    public function create_qrcode_record($user_info, $qrcode, $wx_data) {
        ...
        $value = array(
            ...
            'type' => $wx_data['type'],
            'wx_data'=>json_encode($wx_data ? $wx_data: $user_info),
            'create_time'=>TIMESTAMP,
            'date'=>date('Ymd'),
        ...
        pdo_insert('ching_ifollow_record', $value);
        return true;
    }
    ...
    public function create_qrcode_fans($user_info, $qrcode, $message) {
        ...
        pdo_insert('ching_ifollow_fans', $value);
        return true;
    }
    ...
    public function update_fans($open_id, $type, $time) {
        ...
        //pdo_update('ching_ifollow_fans', array('follow'=>2), $where);
        $up_data = array();
        if($type == 'subscribe') {
            $up_data['follow'] = 1;
        ...
        if($up_data) {
            pdo_update('ching_ifollow_fans', $up_data, $where);
        }
        $fans = pdo_get('ching_ifollow_fans', $where);
        //更新分组
        load()->classs('account');
        $account = WeAccount::create($_W['acid'] ? $_W['acid'] : $_W['uniaccid']);
        $account->updateFansGroupid($open_id, $fans['group_id']);
        return true;
    }
    ...
    public function qrcode_stat($qrcode, $message, $user_info) {
        ...
        $uniacid = $_W['uniacid'];
        //统计组关注人数
        $group_num = $this->pdo_select_count('ching_ifollow_fans', array('uniacid'=>$uniacid, 'group_id'=>$qrcode['group_id'], 'follow'=>1));
        pdo_update('ching_ifollow_fans_group', array('count' => $group_num), array('mc_group_id' => $qrcode['group_id']));
        //统计取消关注数量
        $cancel_group_num = $this->pdo_select_count('ching_ifollow_fans', array('uniacid'=>$uniacid, 'group_id'=>$qrcode['group_id'], 'follow'=>2));
        pdo_update('ching_ifollow_fans_group', array('cancel_count' => $cancel_group_num), array('mc_group_id' => $qrcode['group_id']));
        //统计扫描数
        pdo_update('ching_ifollow_fans_group', array('scan_count +=' => 1), array('mc_group_id' => $qrcode['group_id']));
        //统计二维码
        $qrcode_subnum = $this->pdo_select_count('ching_ifollow_fans', array('uniacid'=>$uniacid, 'group_id'=>$qrcode['group_id'], 'qrcode_id'=>$qrcode['id'], 'follow'=>1));
        $qrcode_cancelnum = $this->pdo_select_count('ching_ifollow_fans', array('uniacid'=>$uniacid, 'group_id'=>$qrcode['group_id'], 'qrcode_id'=>$qrcode['id'], 'follow'=>2));
        //扫码数
        $qrcode_scannum = $this->pdo_select_count('ching_ifollow_record', array('uniacid'=>$uniacid, 'group_id'=>$qrcode['group_id'], 'qrcode_id'=>$qrcode['id'], 'type'=>'qr'));
        $up_qrcode = array(
        ...
        pdo_update('ching_ifollow', $up_qrcode, array('id'=>$qrcode['id']));
        //画出统计数据
        $stat_where = array(
            'uniacid'=>$uniacid,
            'qrcode_id'=>$qrcode['id'],
        ...
        $stat_info = pdo_get('ching_ifollow_stat', $stat_where);
        //扫码数
        $qrcode_scan_num = $this->pdo_select_count('ching_ifollow_record', array_merge($stat_where, array('date'=>date('Ymd'))));
        //关注数
        $qrcode_sub_num = pdo_fetchcolumn('select count(id) from '.tablename('ching_ifollow_fans').' where uniacid=:uniacid and qrcode_id=:qrcode_id and group_id=:group_id and follow=:follow and create_time>:create_time', array(
            ':uniacid'=>$uniacid,
            ':qrcode_id'=>$qrcode['id'],
            ':group_id'=>$qrcode['group_id'],
            ':follow'=>1,
        ...
        //取消关注数
        $qrcode_cancel_num = pdo_fetchcolumn('select count(id) from '.tablename('ching_ifollow_fans').' where uniacid=:uniacid and qrcode_id=:qrcode_id and group_id=:group_id and follow=:follow and create_time>:create_time', array(
            ':uniacid'=>$uniacid,
            ':qrcode_id'=>$qrcode['id'],
            ':group_id'=>$qrcode['group_id'],
            ':follow'=>2,
        ...
        pdo_insert('log', array('cont'=>var_export(array($qrcode_sub_num,$qrcode_cancel_num), true)));
        $stat_value = array(
        ...
        if(!$stat_info) {
            ...
            pdo_insert('ching_ifollow_stat', array_merge($stat_where, $stat_value));
        } else {
            $stat_value['update_time'] = TIMESTAMP;
            pdo_update('ching_ifollow_stat', $stat_value, array('id'=>$stat_info['id']));
        }
        return TRUE;
    }
    ...
    protected function pdo_select_count($tablename, $params = array()){
        $condition = $this->ching_implode($params, 'AND');
    ...
    protected function ching_implode($params, $glue = ',') {
