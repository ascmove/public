<?php
    ...
    $fansgroup = pdo_getall('ching_ifollow_fans_group', array('uniacid'=>$_W['uniacid']), array('id', 'mc_group_id', 'name'));
    if(!$op) {
        ...
        $size = 10;
        $total = count(pdo_getall('ching_ifollow',$where, array('id')));
        $list = pdo_getslice('ching_ifollow', $where,  "LIMIT " . ($page - 1) * $size . " , " . $size, $total);
        if($list) {
            foreach($list as $k=>$v) {
                $r = pdo_get('ching_ifollow_fans_group', array('mc_group_id'=>$v['group_id']), array('name'));
    ...
    if($op == 'create') {
        if(checksubmit()){
            ...
            if(!is_dir(IA_ROOT.'/attachment/'.CHING_NAME.'/'.date('Y').'/'.date('m').'/'.date('d'))) {
                @mkdirs(IA_ROOT.'/attachment/'.CHING_NAME.'/'.date('Y').'/'.date('m').'/'.date('d'), '0777');
            }
            if(!$_GPC['group_id']) {
            ...
            $info = pdo_fetch( 'SELECT * FROM '.tablename('ching_ifollow').' WHERE uniacid = :uniacid AND id = :id' , array(':uniacid' => $uniacid,':id'=>$id));
            if( $info ){
        ...
        include ching_template('qrcode/create');
        die;
    } elseif($op == 'post') {
        //二维码编辑
        $id = intval($_GPC['id']);
        $row = pdo_get('ching_ifollow', array('id'=>$id));
        if(checksubmit()) {
            $name = trim($_GPC['name']);
            pdo_update('ching_ifollow', array('name'=>$name), array('id'=>$id));
            message('保存成功', 'refresh');
        }
        include ching_template('qrcode/create');
    } elseif($op == 'del') {
        ...
        pdo_delete('ching_ifollow', array('id'=>$id));
        if($row['rid']) {
            $rid = $row['rid'];
            load()->model('reply');
