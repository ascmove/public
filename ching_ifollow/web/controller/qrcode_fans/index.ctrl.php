<?php
    ...
    if(!$op) {
        ...
        $size = 10;
        if($_GPC['act'] == 'export') {
            $_list = pdo_getall('ching_ifollow_fans', $where, array('id', 'nickname', 'sex','group_id', 'qrcode_id', 'follow', 'subscribe_time', 'cancel_sub_time'));
            $columns = array(
            ...
            if($_list) {
                ...
                foreach ($_list as $k => $item) {
                    ...
                    $g = pdo_get('ching_ifollow_fans_group', array('id'=>$item['group_id']), array('name'));
                    $r = pdo_get('ching_ifollow', array('id'=>$item['qrcode_id']), array('name'));
        ...
        $total = count(pdo_getall('ching_ifollow_fans',$where, array('id')));
        $list = pdo_getslice('ching_ifollow_fans', $where,  "LIMIT " . ($page - 1) * $size . " , " . $size, $total);
        if($list) {
            foreach($list as $k=>$v) {
                $g = pdo_get('ching_ifollow_fans_group', array('mc_group_id'=>$v['group_id']), array('name'));
                $r = pdo_get('ching_ifollow', array('id'=>$v['qrcode_id']), array('name'));
    ...
    include ching_template('qrcode_fans/index');
