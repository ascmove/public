<?php
    ...
    if($operation == 'display') {
        ...
        $list = pdo_getall('ching_ifollow_fans_group', $where);
    }
    load()->classs('account');
    ...
    if($operation == 'edit') {
        ...
        if(checksubmit()) {
            ...
            if(!$_GPC['group_id']) {
                ...
                pdo_insert('ching_ifollow_fans_group', $qrcode_group);
            } else {
                pdo_update('ching_ifollow_fans_group', $qrcode_group, array('id'=>$_GPC['group_id']));
            }
            message('保存分组名称成功', web_url('qrcode_fans_group/index'), 'success');
        }
        if($_GPC['id']) {
            $item = pdo_get('ching_ifollow_fans_group', array('uniacid'=>$_W['uniacid'], 'id'=>intval($_GPC['id'])));
        }
    } elseif($operation == 'del') {
        //删除分组
        $groups = pdo_get('ching_ifollow_fans_group', array('id'=>intval($_GPC['id'])));
        ...
        pdo_delete('ching_ifollow_fans_group', array('id' => intval($_GPC['id'])));
    ...
    if($operation == 'sync') {
        ...
        if($groups) {
            foreach($groups as $group) {
                if($res = pdo_get('ching_ifollow_fans_group', array('name'=>$group['name'], 'uniacid'=>$_W['uniacid']))){
                    $up_data = array(
                        'count'=>$group['count'],
                        'mc_group_id'=>$group['mc_group_id']
                    );
                    pdo_update('ching_ifollow_fans_group', $up_data, array('id'=>$res['id']));
                } else {
                    $inser_data = array(
                        'uniacid'=>$_W['uniacid'],
                        'name'=>$group['name'],
                    ...
                    pdo_insert('ching_ifollow_fans_group', $inser_data);
                }
            }
        }
    ...
    include ching_template('qrcode_fans_group/index');