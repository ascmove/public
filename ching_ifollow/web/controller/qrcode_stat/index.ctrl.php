<?php
    ...
    $_group = pdo_getall('ching_ifollow_fans_group', $where);
    if($_GPC['group_id']) {
        $where['mc_group_id'] = intval($_GPC['group_id']);
    }
    $group = pdo_getall('ching_ifollow_fans_group', $where);
    $_group = pdo_getall('ching_ifollow_fans_group', array('uniacid'=>$_W['uniacid']));
    $qrcode = pdo_getall('ching_ifollow', array('uniacid'=>$_W['uniacid']), array('id', 'name'));
    ...
    if($operation == 'display') {
        ...
        $sum = pdo_fetch('select sum(scan_count) as scan_count,sum(sub_num) as sub_num,sum(cancel_num) as cancel_num from '.tablename('ching_ifollow_stat').' where '.$where, $where_arr);
        extract($sum);
        $dayarr = array();
            ...
            foreach($group as $item) {
                if($dayarr) {
                    foreach($dayarr as $day) {
                        ...
                        $sum_num = pdo_fetch('select sum(sub_num) as sub_num from '.tablename('ching_ifollow_stat').' where '.$map_str, $map);
    ...
    elseif($operation == 'top') {
        ...
        $list = pdo_fetchall('select * from '.tablename('ching_ifollow').' where uniacid=:uniacid order by subnum desc, cancelnum desc', array(':uniacid'=>$_W['uniacid']));
        include ching_template('qrcode_stat/top');
        die;
    }
    include ching_template('qrcode_stat/index');